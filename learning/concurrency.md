...
На Kotlin/Native `Default` — один фоновый поток, `Main` привязан к основному треду через RunLoop.

### withContext

Переключает контекст (обычно диспатчер) для блока кода и возвращает результат. Не создаёт новой корутины в отличие от `async` — просто меняет поток внутри существующей.

```kotlin
scope.launch(Dispatchers.Main) {
    val data = withContext(Dispatchers.Default) { loadData() }
    // снова Main, обновляем UI
    render(data)
}
```
...
Канал для передачи данных между корутинами (горячий стрим). Producer отправляет, consumer получает. Разные типы буферизации: `RendezVous` (по умолчанию — без буфера, отправитель ждёт получателя), `Buffered`, `Conflated`, `Unlimited`.
...
Холодный асинхронный стрим данных. Flow ничего не испускает, пока на него не подписались. В отличие от Channel, Flow — ленивый и иммутабельный.
...
**StateFlow** — горячий стрим, хранит последнее значение. Аналог `LiveData` в Android, идеален для UI-состояния:
...
**SharedFlow** — горячий стрим без состояния. Для одноразовых событий (тосты, навигация). Используй с `replay` для опоздавших подписчиков.
...
Неблокирующий мьютекс для корутин — защита shared mutable state. Захват через `withLock` приостанавливает корутину, а не поток.
...
Всегда используй `withLock` вместо ручного `lock()` / `unlock()`. Mutex — shareable, его можно замёрзнуть и передать между потоками.
...
С новым менеджером памяти (стабилен с Kotlin 1.9.20) freeze **не нужен** — объекты безопасно передаются между потоками без заморозки. Код с `freeze()` всё ещё будет компилироваться как no-op.
