...
Бывают ситуации, что при компиляции KMP проекта Gradle daemon неожиданно завершается (крешится). Причины такого поведения могут быть разные и их сложно разбирать, поэтому в данной статье описано как можно анализировать данную проблему на нескольких ситуациях, с которыми мы столкнулись в своей работе.
...
Данная версия лога ничего не объяснила, также как и при использовании опции `--stacktrace` (ведь демон крешится). Поэтому следующий шаг - выполнить компиляцию с опцией `-i` - в логе будет очень много сообщений, но главное что в логе будут и команды которые вызывает gradle при обращении к внешним приложениям (например к компилятору Kotlin/Native). 

Запускаем задачу, которая крешнулась, с опцией `-i` (через терминал):
```
./gradlew linkMultiPlatformLibraryDebugFrameworkIosArm64 -i
```

В конце лога, почти перед тем как происходит креш, видно команду для `konan` (это название Kotlin/Native компилятора) - мы взяли ее из лога и привели к виду, который можно воткнуть в Terminal для запуска:
```
./konanc -g \
        -ea \
        -target \
