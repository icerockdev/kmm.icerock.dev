...
Примеры: значение текстового поля, включена кнопка или нет, заголовок на экране и т.д.
...
- кнопка "Зарегистрироваться" `val isRegisterButtonEnabled: LiveData<Boolean>`
...
Для начала, представим, что у нас есть `viewModel`, в которой есть следующие лайвдаты, относящиеся к загрузке какой-нибудь страницы из интернета:
...
В добавок, при переходе от стейта к стейту, мы могли бы забыть изменить или скрыть какой-нибудь элемент, после чего долго и внимательно отсматривали бы каждый кейс `when`-а в поисках ошибки.
...
Разберем пример. Со стороны вьюмодели у нас будет `Channel`, который публично будет виден как `Flow`. Со стороны `UI` мы подпишемся к нему и будем обрабатывать события.
...
- `trySend` же возвращает `Boolean`: `true` - если добавить новое значение удалось, `false` - если не удается добавить из-за превышения объема буфера значений. Это значит, что если в очереди уже есть какие-то события, которые не успел получить UI, то новое просто будет утеряно. Поэтому всегда следует использовать `send`.
...
Ранее событиями занимался класс EventsDispatcher. Подход с использованием EventsDispatcher считается устаревшим, в новых проектах мы его не используем. Ниже справочная информация на случай, если встретитесь с ним.
