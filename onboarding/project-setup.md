...
Для компиляции iOS-приложения, а также Kotlin-библиотеки для iOS, потребуется Xcode. Вместо долгой загрузки из AppStore рекомендуем использовать [Xcodes](https://www.xcodes.app/) — удобный менеджер версий Xcode с графическим интерфейсом.
