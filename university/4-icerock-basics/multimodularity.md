...
- Разработчик сделал изменение в `public api` модуля `F`. Например, в публичной функции какого-нибудь класса модуля F, которую, соответвенно, могут использовать за пределами файла, в котором она объявлена  
- Разработчик сделал изменение в `private api` модуля `F`.Например, в приватной функции какого-нибудь класса модуля F, которая, соответвтенно, может быть использована только в файле, в котором она объявлена
...
- ***Функционал, общий для нескольких фичей***  
    ...
    - Если в этом функционале есть какие-то зависимости от других модулей, то такой функионал следует подлкючать в mppLibrary к нужным фичам как `Dependency injection` или `Dependency injection`   
