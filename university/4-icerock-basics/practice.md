...
15. При перезапуске приложения авторизация должна сохраняться
16. Использовать локализацию для всех строк, показываемых пользователю
17. Использовать векторную графику везде, где это возможно
18. Обеспечить поддержку Android API 21
19. Локализовать проект используя `sheets-localizations-generator`
    - обеспечьте поддержку русского и английского языков
20. Обеспечить поддержку iOS 13.0
