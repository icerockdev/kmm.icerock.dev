...
10. Не создавайте дополнительную переменную, если она никак не улучшает читаемость кода
    - не стоит исопльзовать `binding`
       ```kotlin
       with(binding) {
           somethingButton.text = TODO()
           somethingButton.setOnClickListener { TODO() }
           label1.text = TODO()
           label2.text = TODO()
           label3.text = TODO()
       }
       ```
    - соит исопльзовать `binding`
       ```kotlin
...
5. Верстайте экран сверху вниз, констрейнты вьюхам устанавливайте относительно друг друга, а не привязывайте каждую к корневому `Layout`
   - если элемент всегдя расположен внизу экрана - не надо цеплять его к верхенему элементу
