...
Ошибки структурированы по трём частям
- Common
- iOS 
- Android
...
16. Используйте аннотации `@Throws(...)` только в случае, если это попадает в swift код. [документация](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin/-throws/)
...
23. Не забывайте про конфликты имен Kotlin и iOS
...
8. Все `ViewController`-ы должны называться с окончанием `ViewController`, а не `Screen`, потому что `Screen`- это экран девайса. `ViewController` не обязательно занимает весь экран, их на экране сразу несколько: твой собственный, `UINavigationController`, ***еще несколько добавить***
