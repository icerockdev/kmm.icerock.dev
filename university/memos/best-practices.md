...
- не надо выдавать никакие данные из юнита (например по клику)
- не создвавайте общий класс констант - хламовник, константы должны создаваться там, где они имеют смысл
- Никогда не обрабатывайте Error-ы. у Throwable два наследника - Exception и Error. Эксепшены надо обрабатывать, они исправимы. а Error'ы - неисправимы. их не надо обрабатывать, приложение должно упасть с информацией о том чего пошло не так (в эксепшене эта информация)
...
- все ViewControllerы называй с окончанием ViewController. Screen это иное - это экран девайса. а вьюконтроллер не обязательно весь экран занимает. их куча на экране сразу (тот же UINavigationController вспомни)
...
- укажи тему NoActionBar и вместо ActionBar'а системного используй Toolbar - UI элемент который полностью тобой управляется. у него есть и интеграция с Navigation Component встроенная.
- константы класса - в companionObject. В свойствах должно быть все то, что уникальное для каждого экземпляра класса
